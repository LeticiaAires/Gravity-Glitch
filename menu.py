...
def display_rules():
    ...
    font_filename = "your_font.ttf"
        # Full path to your font file
    font_path = os.path.join("C:\Users\zinef\Downloads\GAME\2324_Projet2A_JeuVideo", font_filename)
        # Load custom font for the title of the Rules
    title_setting_font = pygame.font.Font(font_path, 50)
    title_setting_text = title_setting_font.render(" Rules ", True, (255,255,255))
    title_setting_rect = title_setting_text.get_rect(center=(SCREEN_WIDTH // 2, 50))
    ...
    # Create text surfaces
    setting_font = pygame.font.Font(font_path, 40)
    rule1_text = setting_font.render(" Try to get the bird through as many holes as possible ", True, (0, 0, 0))
    rule2_text = setting_font.render(" If you hit the pipes the game is over ", True, (0, 0, 0))
    rule3_text = setting_font.render(" To play, you have the space bar or the console to make the bird fly  ", True, (0, 0, 0))

    # Set positions for the text
    rule1_rect = rule1_text.get_rect(center=(SCREEN_WIDTH // 2, 100))
    rules2_rect = rule2_text.get_rect(center=(SCREEN_WIDTH // 2, 200))
    rules3_rect = rule3_text.get_rect(center=(SCREEN_WIDTH // 2, 300))
    while running1:  
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running1 = False
        ...
        if return_rect.collidepoint(mouse_pos1):
            ...
            # Update the display
        screen.blit(background_setting_image, (0, 0))
        screen.blit(title_setting_font.render(" Rules ", True, (0, 0, 0)), (SCREEN_WIDTH // 3 - 200, 30))
        screen.blit(return_button, (return_rect.centerx - return_button.get_width() // 2, return_rect.centery - return_button.get_height() // 2))
        pygame.display.update()
    pygame.mixer.quit()
    pygame.quit()
